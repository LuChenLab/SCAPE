...
def parse_cfgfile(cfg_file):
    ...
    for line in open(cfg_file, 'r'):
        if line[0] == '\n' or line[0] == '#':
            ...
            if command[0] == 'chromsome_id':
                chromsome_id = command[1].split(',')
    ...
    return Aligned_Wig_files, output_directory, Annotated_3UTR_file, Output_result_file, sequencing_depth_file, Num_threads, Coverage_threshold, chromsome_id
...
def run(argv=None):
    ...
    start_time = time_now()
    Group1_Tophat_aligned_file, output_directory, Annotated_3UTR_file, Output_result_file, sequencing_depth_file, Num_threads, Coverage_threshold, chromsome_id = parse_cfgfile(cfg_file)
    ...
    for curr_processing_chr in chromsome_id:
        Output_all_prediction_file = output_directory + Output_result_file + curr_processing_chr + '.txt'
    ...
    end_time = time_now()
    with open(done_file, "w") as done_fh:
        done_fh.write(str(start_time) + "\n")
        done_fh.write(str(end_time) + "\n")
